...
export function buildWorkOrderExportHtml(data: {
  ...
  templateString = templateString
    ...
    .replace(/\{\{orderId\}\}/g, documentNumber)
    .replace(/\{\{offerId\}\}/g, documentNumber)
    .replace(/\{\{customerFullName\}\}/g, String(offer.customerFullName ?? ''))
    .replace(/\{\{yachtName\}\}/g, yacht.yachtName)
    .replace(/\{\{yachtModel\}\}/g, yacht.yachtModel)
    .replace(/\{\{countryCode\}\}/g, yacht.countryCode)
    .replace(/\{\{location\}\}/g, String(offer.location ?? ''))
    .replace(/\{\{createdAt\}\}/g, createdAtString)
    .replace(/\{\{assignedWorkers\}\}/g, assignedWorkers || '—')
